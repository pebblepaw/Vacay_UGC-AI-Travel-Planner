...
async def process_workspace_videos(workspace_id: str, request: VideoProcessRequest):
    ...
    video_metadata = [
        {
            "url": r.get("url") or request.urls[i] if i < len(request.urls) else "",
    ...
    if existing_trip:
        merged_videos = existing_trip.source_videos + [v for v in new_trip.source_videos if v.url not in {ev.url for ev in existing_trip.source_videos}]
        merged_days = existing_trip.days + new_trip.days
        existing_trip.source_videos = merged_videos
        existing_trip.days = merged_days
        merged_trip = existing_trip
    else:
        merged_trip = new_trip

    await storage.save_trip(merged_trip)
    await workspace_runtime.append_event(workspace_id, "agent", f"Imported {len(successful)} new media links into workspace")
    snapshot = await workspace_runtime.build_workspace_snapshot(workspace_id, merged_trip)

    return {"workspace_id": workspace_id, "trip_id": merged_trip.trip_id, "snapshot": snapshot}
...
async def _invoke_workspace_agent(workspace_id: str, message: str, user_id: str | None, source: str) -> ChatResponse:
    ...
    if not trip:
        ...
        await workspace_runtime.ensure_workspace(workspace_id, title=trip.title)
    ...
    await workspace_runtime.save_runtime_state(workspace_id, {
    ...
    # Workspace and user memory (simple key/value capture)
