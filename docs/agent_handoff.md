...
- Telegram webhook ingestion exists at `/api/telegram/webhook`, keyed by `chat_id + message_thread_id` workspace IDs.
- Frontend supports workspace mode via `/?workspace=<id>&token=<signed-token>`.
- Workspace snapshots now drive live polling updates and media-per-location folders in Cards view.
- Signed share links are available via `/api/workspaces/{workspace_id}/share-link`.
...
1. **Real Telegram webhook roundtrip** (no live Telegram bot token + public webhook URL in this VM).
...
3. **Real cloud browser handoff on Trip.com** end-to-end from Telegram event through interactive browser takeover.
